...
public class BurningPower extends AbstractPower
    ...
    public static final String POWER_ID = HumanMod.makeID("BurningPower");
    ...
    private static final Texture tex84 =
        TextureLoader.getTexture(makePowerPath("BurningPower_84.png"));
    private static final Texture tex32 =
        TextureLoader.getTexture(makePowerPath("BurningPower_32.png"));
    public AbstractCreature source;

    public BurningPower(final AbstractCreature owner,
    ...
    public void atStartOfTurn() {
        ...
        for (int i = 0; i < amount; i++) {
            this.addToBot(
                new LoseHPAction(owner, owner, MathUtils.random(2, 5)));
        }
        if (this.amount == 1) {
    ...
    public void atEndOfTurn(boolean isPlayer) {
        this.flash();
        this.addToBot(new LoseHPAction(owner, owner, MathUtils.random(2, 5)));
        if (this.amount == 1) {
            this.addToBot(new RemoveSpecificPowerAction(owner, owner, this));
        } else {
            this.addToBot(new ReducePowerAction(owner, owner, this, 1));
