...
public class PlayGhostCopyOfCardAction extends AbstractGameAction {
	...
	public PlayGhostCopyOfCardAction(AbstractCard c, AbstractMonster target, int times) {
		this.card = c;
		this.target = target == null ? AbstractDungeon.getRandomMonster() : target;
		this.times = times;
	...
	public void update() {
		if (!this.isDone) {
			...
			for (int i = 0; i < this.times; i++) {
				AbstractDungeon.actionManager.addCardQueueItem(new CardQueueItem(tmp, (AbstractMonster) this.target, 0, true, true), true);
			}
			this.isDone = true;
		}
		this.tickDuration();
