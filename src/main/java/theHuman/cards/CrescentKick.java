...
public class CrescentKick extends AbstractDynamicCard {
	...
	public static final String ID = HumanMod.makeID(CrescentKick.class.getSimpleName());
	...
	public void use(AbstractPlayer p, AbstractMonster m) {
		...
		if (p.hasPower(TechniquePower.POWER_ID)) {
			while (count > 0) {
				this.addToBot(new DamageAllEnemiesAction(p, damage, damageTypeForTurn, AbstractGameAction.AttackEffect.SMASH));
				count--;
			}
		} else {
			while (count > 0) {
